...
CREATE TABLE IF NOT EXISTS customer_profiles (
  ...
  live_latitude DECIMAL(10,8) NULL,
  live_longitude DECIMAL(11,8) NULL,
  live_location_at DATETIME NULL,
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS bookings (
  ...
  start_otp_expires_at DATETIME NULL,
  finish_otp VARCHAR(6) NULL,
  finish_otp_expires_at DATETIME NULL,
  started_at DATETIME NULL,
  finish_requested_at DATETIME NULL,
  remaining_payment_method ENUM('cash','online') NULL,
  razorpay_remaining_order_id VARCHAR(100) NULL,
  razorpay_remaining_payment_id VARCHAR(100) NULL,
  status ENUM('payment_pending','pending','confirmed','in_progress','awaiting_payment','cancelled','completed') DEFAULT 'payment_pending',
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY (customer_id) REFERENCES users(id) ON DELETE CASCADE,
