...
import { calculateBookingPrice, formatINR, SAMAGRI_RATE } from '@/lib/booking-pricing';
...
export function BookPanditScreen({ panditProfileId, initialServiceName }: BookPanditScreenProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Booking Submitted', response.message || 'Your booking request has been sent.', [
  ...
  return (
    <View style={styles.root}>
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        >
          ...
          <View style={styles.priceCard}>
            ...
            <PriceRow label="Total Amount" value={formatINR(pricing.totalPrice)} bold />
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
      ...
      <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, 12) }]}>
        <Pressable
          style={[styles.submitBtn, createBooking.isPending && styles.submitBtnDisabled]}
        ...
        >
          {createBooking.isPending ? (
            ...
            <>
              ...
              <Text style={styles.submitText}>Confirm Booking • {formatINR(pricing.totalPrice)}</Text>
            </>
          )}
        </Pressable>
      </View>
